...
async function Engine({
  ...
  spinnies.add(metaSpin, {
    text: (/* @__PURE__ */ new Date()).toLocaleString() + " " + colors14.bold.blue("INFO: ") + colors14.reset("")
  });
  spinnies.update(metaSpin, {
    text: (/* @__PURE__ */ new Date()).toLocaleString() + " " + colors14.bold.blue("INFO: ") + "\u2B55 using yt-core version <(" + version + ")>" + colors14.reset("")
  });
  if (!query || query.trim() === "") {
    spinnies.fail(metaSpin, {
      text: (/* @__PURE__ */ new Date()).toLocaleString() + " " + colors14.bold.red("ERROR: ") + "\u274C 'query' is required..." + colors14.reset("")
    });
    return;
  }
  if (/https/i.test(query) && /list/i.test(query)) {
    spinnies.fail(metaSpin, {
      text: (/* @__PURE__ */ new Date()).toLocaleString() + " " + colors14.bold.red("ERROR: ") + "\u274C use extract_playlist_videos() for playlists..." + colors14.reset("")
    });
    return;
  }
  if (/https/i.test(query) && !/list/i.test(query)) {
    spinnies.update(metaSpin, {
      text: (/* @__PURE__ */ new Date()).toLocaleString() + " " + colors14.bold.blue("INFO: ") + "\u2B55 fetching metadata for: <(" + query + ")>" + colors14.reset("")
    });
  ...
  if (videoId) {
    ...
    if (!TubeBody.stdout.Link) {
      spinnies.fail(metaSpin, {
        text: (/* @__PURE__ */ new Date()).toLocaleString() + " " + colors14.bold.red("ERROR: ") + "\u274C no data returned from server..." + colors14.reset("")
      });
      return;
    } else {
      spinnies.update(metaSpin, {
        text: (/* @__PURE__ */ new Date()).toLocaleString() + " " + colors14.bold.blue("INFO: ") + "\u{1F4E1} preparing payload for <(" + TubeBody.stdout.Title + "Author:" + TubeBody.stdout.Uploader + ")>" + colors14.reset("")
      });
  ...
  if (TubeCore === null) {
    spinnies.fail(metaSpin, {
      text: (/* @__PURE__ */ new Date()).toLocaleString() + " " + colors14.bold.red("ERROR: ") + "\u274C please try again later..." + colors14.reset("")
    });
    return;
  }
  spinnies.succeed(metaSpin, {
    text: (/* @__PURE__ */ new Date()).toLocaleString() + " " + colors14.bold.blue("INFO: ") + "\u2763\uFE0F Thank you for using yt-core! If you enjoy the project, consider Staring the GitHub repo https://github.com/shovitdutta/mixly/yt-core." + colors14.reset("")
  });
...
var progressBar = (percent) => {
  ...
  const completedStr = colors14.bold.bgGreen(" ".repeat(completed));
  const incompleteStr = colors14.bold.gray(" ".repeat(incomplete));
  const percentStr = colors14.bold.blue(" " + percent.toFixed(2) + "% ");
  const progressBarString = (/* @__PURE__ */ new Date()).toLocaleString() + colors14.bold.blue(" INFO:") + " " + completedStr + incompleteStr + percentStr;
